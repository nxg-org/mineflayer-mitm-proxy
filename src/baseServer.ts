...
import prisLoader from "prismarine-registry";
type Registry = ReturnType<typeof prisLoader> & { supportFeature: (feature: string) => boolean };
/**
...
export type Test<Events> = {
    [nme in keyof Events as nme extends string ? `on${Capitalize<nme>}` : never]?: Events[nme] extends (...args: infer Args) => infer Ret
        ? (...args: Args) => Ret
        : never;
};
...
export class ServerBuilder<Opts extends IProxyServerOpts, Emits extends IProxyServerEvents, AppliedSettings = false> {
    ...
    >(this: ServerBuilder<Opts, Emits, AppliedSettings>, ...plugins: Plugins): ServerBuilder<Opts & O, Emits & E, NeedsSettings> {
    ...
    public setSettings(settings: Opts & Record<string, any>): ServerBuilder<Opts, Emits, true> {
...
> extends TypedEventEmitter<Events> {
    ...
    private _foundVersion: string;
    private _cachedData: Registry;
    ...
    protected notConnectedLoginHandler = async (actualUser: ServerClient) => {
        ...
        if (this.refData.supportFeature("usesLoginPacket")) {
            const data = Object.assign({}, this.refData.loginPacket);
            data.entityId = actualUser.id;
            // TODO: hotfix mcData's broken dimensionCodec.
            if (this.refData.supportFeature("dimensionDataInCodec")) {
                (data.dimensionCodec! as any).value = {};
            }
            actualUser.write("login", data);
        } else {
